...
> 4. **Visual & Generative Transparency**: Grad-CAM/Grad-CAM++ spatial heatmaps and RAG-grounded care protocols (100.0% structured knowledge base retrieval coverage across 18 benchmark disease queries).
...
| **Grounding Ablation (RAG Retrieval)** | Strategy A 100.0% KB coverage (Strategy B human evaluation PENDING) | `results/week2/grounding_ablation_results.json` |
...
**Answer**:
"Direct LLM prompting is vulnerable to chemical dosage hallucinations and unverified remedies when generating agricultural advice. We implement a Retrieval-Augmented Generation (RAG) engine that fetches verified agronomic protocols (symptoms, chemical active ingredients, organic remedies, prevention) from `rag_knowledge_base.json` and injects them as strict context in the LLM prompt. Across our 18 benchmark queries, Strategy A (Grounded RAG) achieved 100.0% structured knowledge-base retrieval coverage, while an interactive human evaluation pass for direct LLM generation remains pending manual execution."
